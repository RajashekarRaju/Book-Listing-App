...
class QueryUtils {
    ...
    private static List<Book> extractFeatureFromJson(String bookJSON) {
        ...
        try {
            ...
            for (int i = 0; i < bookArray.length(); i++) {
                ...
                // Extract the value for the key called "smallThumbnails"
                String image = null;
                if (properties.has("imageLinks")) {
                    JSONObject images = properties.getJSONObject("imageLinks");
                    image = images.optString("smallThumbnail");
                }


                // Extract the value for the key called "previewLink"
                String url = null;
                if (properties.has("previewLink")) {
                    url = properties.getString("previewLink");
                }

                // Create a new {@link Book} object with the magnitude, location, time,
                // and url from the JSON response.
                Book book = new Book(title, author, publisher, image, url);


                // Add the new {@link Book} to the list of book.
                books.add(book);
            }
    ...
    /**
     * Query the Books API dataset and return a list of {@link Book} objects.
     */
